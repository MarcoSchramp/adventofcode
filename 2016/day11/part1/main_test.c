...
void printsystemstate(struct systemstate* systemstate)
{
	for (int i = 0; i < MAXFLOORS; i++) {
		printf ("| %2o %2o |", systemstate->floorstate[i].chip_state, systemstate->floorstate[i].generator_state);
	}
	printf("Floor: %d\n", systemstate->floor);
}
	...
	for (struct takealong *takealong = takealonglist; takealong; takealong = takealong->next) {
		if (((systemstate->floorstate[floor].chip_state & takealong->floorstate.chip_state) == takealong->floorstate.chip_state) &&
			...
			if (floor > 0) {
				...
				if (issafematrix[targetsystemstate.floorstate[floor-1].chip_state][targetsystemstate.floorstate[floor-1].generator_state]) {
...
int main(int argc, char* argv[])
{
	...
	initialsystemstate.floorstate[2].chip_state =  LITHIUM;
