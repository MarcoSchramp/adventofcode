...
int longest = 0;
int longestpath[MAXCITY];
int currentpath[MAXCITY];

void findlongestpath(int level, int distance, int currentcity, int remaining[MAXCITY])
{
	int found  = 0;
	for (int i = 0; i < citycount; i++) {
		if (remaining[i] == 0) {
			...
			findlongestpath(level + 1, distancein, i, remaining);
	...
	if (!found ) {
		if (distance > longest) {
			longest = distance;
			memcpy(longestpath, currentpath, sizeof longestpath);
		}
	}
}
...
int main(int argc, char* argv[])
{
	...
	findlongestpath(0, 0, 0, remaining);
